...
impl<'a> VSlider<'a> {
    ...
    /// For instance, `y` is the mouse position and `y_range` is the physical location of the slider on the screen.
    fn value_from_y(&self, y: f32, y_range: RangeInclusive<f32>) -> f64 {
        let normalized = remap_clamp(y, y_range, 0.0..=1.0) as f64;
        value_from_normalized(normalized, self.range(), &self.spec)
    }

    fn y_from_value(&self, value: f64, y_range: RangeInclusive<f32>) -> f32 {
        let normalized = normalized_from_value(value, self.range(), &self.spec);
        lerp(y_range, normalized as f32)
    }
}
...
fn y_range(rect: &Rect) -> RangeInclusive<f32> {
...
impl<'a> VSlider<'a> {
    ...
    fn allocate_slider_space(&self, ui: &mut Ui, width: f32) -> Response {        
        let desired_size = vec2(width, ui.spacing().slider_width+15.0);
    ...
    fn slider_ui(&mut self, ui: &mut Ui, response: &Response) {
        ...
        let y_range = y_range(rect);

        if let Some(pointer_pos) = response.interact_pointer_pos() {
            let new_value = if self.smart_aim {
                let aim_radius = ui.input().aim_radius();
                emath::smart_aim::best_in_range_f64(
                    self.value_from_y(pointer_pos.y - aim_radius, y_range.clone()),
                    self.value_from_y(pointer_pos.y + aim_radius, y_range.clone()),
                )
            } else {
                self.value_from_y(pointer_pos.y, y_range.clone())
            };
        ...
        if response.has_focus() {
            ...
            if kb_step != 0.0 {
                ...
                let prev_y = self.y_from_value(prev_value, y_range.clone());
                let new_y = prev_y + kb_step;
                let new_value = if self.smart_aim {
                    let aim_radius = ui.input().aim_radius();
                    emath::smart_aim::best_in_range_f64(
                        self.value_from_y(new_y - aim_radius, y_range.clone()),
                        self.value_from_y(new_y + aim_radius, y_range.clone()),
                    )
                } else {
                    self.value_from_y(new_y, y_range.clone())
                };
                self.set_value(new_value);
            }
        }
        ...
        // Paint it:
        {
            ...
            let rail_radius = ui
                .painter()
                .round_to_pixel((rect.width() / 4.0).at_least(2.0));

            let rail_rect = Rect::from_min_max(
                pos2(rect.center().x - rail_radius,rect.top()),
                pos2(rect.center().x + rail_radius, rect.bottom()),
            );
            let marker_center_y = self.y_from_value(value, y_range);
            ...
            ui.painter().add(Shape::Circle {
                center: pos2(rail_rect.center().x, marker_center_y),
    ...
    fn current_gradient(&mut self, y_range: &RangeInclusive<f32>) -> f64 {
        // TODO: handle clamping
        let value = self.get_value();
        let value_from_y = |y: f32| self.value_from_y(y, y_range.clone());
        let y_from_value = |value: f64| self.y_from_value(value, y_range.clone());
        let left_value = value_from_y(y_from_value(value) - 0.5);
        let right_value = value_from_y(y_from_value(value) + 0.5);
        right_value - left_value
    }
}

impl<'a> Widget for VSlider<'a> {
    fn ui(mut self, ui: &mut Ui) -> Response {
        let text_style = TextStyle::Button;
        let width = ui
            .fonts()
            .row_height(text_style)
            .at_least(ui.spacing().interact_size.x) /2.0;

        let old_value = self.get_value();

        let inner_response = ui.vertical_centered(|ui| {
            let slider_response = self.allocate_slider_space(ui, width);
            self.slider_ui(ui, &slider_response);

            if self.show_value {
                let y_range = y_range(&slider_response.rect);
                self.value_ui(ui, y_range);
            }
        ...
        if self.get_value() != old_value { response.mark_changed() };
        response
    }
}
